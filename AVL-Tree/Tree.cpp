...
template <class t>
typename Tree<t>::Node *Tree<t>::balanceTree(Node *node)
{
    if (node == NULL)
    {
        return node;
    }
    if (balanceFactor(node) == 2) // left heavy
    {
        if (balanceFactor(node->left) == -1) //  left right case
        {
            node->left = rotateLeft(node->left); // convert to left left case
        }
        return rotateRight(node); // balance the tree
    }
    if (balanceFactor(node) == -2) // right heavy
    {
        if (balanceFactor(node->right) == 1) // right left case
        {
            node->right = rotateRight(node->right); // convert to right right case
        }
        return rotateLeft(node); // balance the tree
    }
    return node;
}

template <class t>
typename Tree<t>::Node *Tree<t>::rotateRight(Node *node) // right rotation
{
    Node *temp = node->left;   
...
template <class t>
typename Tree<t>::Node *Tree<t>::rotateLeft(Node *node) // left rotation
{
...
template <class t>
int Tree<t>::balanceFactor(Node *node) // balance factor of the node (difference between left and right sub trees)
{
    if (node == NULL)
    {
        return 0;
    }
    return TreeDepth(node->left) - TreeDepth(node->right);
}

template <class t>
void Tree<t>::updateHeight(Node *node) // update the height of the node after insertion or deletion
{

    if (node != NULL)
    {
        int lheight = TreeDepth(node->left);
        int rheight = TreeDepth(node->right);
...
typename Tree<t>::Node *Tree<t>::insertTree(Node *node, t data) // helper method to insert the data into the tree using recursion
{
    ...
    node = balanceTree(node); // balance the tree after insertion of the data to maintain AVL property
...
typename Tree<t>::Node *Tree<t>::DeleteFromTree(Node *node, t data) // helper method to delete the data from the tree using recursion
{
    ...
    updateHeight(node);       // update the height of the node
    node = balanceTree(node); // balance the tree after deletion of the data to maintain AVL property
